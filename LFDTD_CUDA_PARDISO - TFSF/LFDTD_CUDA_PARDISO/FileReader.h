...
inline void Sparse_MatrixImport(const std::string& path, int n, int nnz, double* a, int* ia, int* ja)
{
    ...
    {
        std::cerr << "ERROR: Row file fails to open!!\n";
        exit(-1);
    }

    if (a_file.is_open() && ja_file.is_open())
    {
    ...
    {
        std::cerr << "ERROR: Col and val files fails to open!!\n";
        exit(-1);
    }
...
template<typename T> void fill_n(std::unique_ptr<T[]>& P)
{
